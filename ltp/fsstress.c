...
int	truncate_path(pathname_t *, off_t);
...
int main(int argc, char **argv)
{
	...
	fd = creat(buf, 0666);
	if (lseek(fd, (off_t)(MAXFSIZE32 + 1ULL), SEEK_SET) < 0)
...
check_cwd(void)
{
	...
	if (ret != 0) {
		...
		fprintf(stderr, "fsstress: check_cwd stat() returned %d with errno: %d (%s)\n",
...
int
truncate_path(pathname_t *name, off_t length)
{
	char		buf[NAME_MAX + 1];
	pathname_t	newname;
	int		rval;
...
bulkstat1_f(opnum_t opno, long r)
{
	...
	if (good) {
		...
		if (!get_fname(FT_ANYm, r, &f, NULL, NULL, &v))
			append_pathname(&f, ".");
		ino = stat_path(&f, &s) < 0 ? (ino_t)r : s.st_ino;
		check_cwd();
		free_pathname(&f);
	} else {
                /* 
	...
	long			r)
{
	...
	if (fstat(fd1, &stat1) < 0) {
		if (v1)
			printf("%d/%lld: exchangerange read - fstat %s failed %d\n",
	...
	if (fstat(fd2, &stat2) < 0) {
		if (v2)
			printf("%d/%lld: exchangerange write - fstat %s failed %d\n",
